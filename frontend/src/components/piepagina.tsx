...
const Footer = () => {
  ...
  const socialLinks = [
    ...
    { name: 'Twitter', icon: 'M23 3a10.9 10.9 0 01-3.14 1.53 4.48 4.48 0 00-7.86 3v1A10.66 10.66 0 013 4s-4 9 5 13a11.64 11.64 0 01-7 2c9 5 20 0 20-11.5a4.5 4.5 0 00-.08-.83A7.72 7.72 0 0023 3z', href: 'https://x.com/MAYia_Mx' },
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              {socialLinks.map((social) => (
                <a key={social.name} href={social.href} onMouseEnter={() => setHoveredSocial(social.name)} onMouseLeave={() => setHoveredSocial(null)} className="group relative w-11 h-11 rounded-full flex items-center justify-center transition-all duration-300 transform hover:-translate-y-1 hover:scale-110 shadow-md hover:shadow-xl" style={{ background: '#0A0A14', border: '1px solid #2a2a2a' }} aria-label={social.name}>
                  <svg className="w-5 h-5 transition-colors" style={{ fill: '#A4D955' }} viewBox="0 0 24 24" fillRule="evenodd" clipRule="evenodd">
                    <path d={social.icon} />
                  </svg>
