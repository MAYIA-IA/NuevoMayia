...
const GalagaBg = (): JSX.Element => {
  ...
  useEffect(() => {
    ...
    // Use parent's dimensions instead of window
    ...
    // Observar cambios de tamaño del contenedor (para cuando la tarjeta se expande por los filtros)
    ...
    const drawPlayer = (x: number, y: number): void => {
      ...
      const g = ctx.createRadialGradient(0, 10, 0, 0, 10, 4)
    ...
    const drawExplosion = (ex: Explosion): void => {
      ...
      for (const p of ex.particles) {
        const px = p.vx * progress * 25
        const py = p.vy * progress * 25
        ctx.beginPath()
    ...
    const autoAim = (t: number): void => {
      ...
      let tx = nearest.x + Math.sin(t * 0.02) * 30
    ...
    const loop = (): void => {
      ...
      if (enemyMoveTimer > 45) {
        ...
        if (liveEnemies.length > 0) {
          ...
          if ((enemyDir === 1 && maxX > W - 40) || (enemyDir === -1 && minX < 40)) {
            enemyDir *= -1
            enemies.forEach(e => { if (e.alive) e.y += 12 })
          }
          enemies.forEach(e => { if (e.alive) e.x += enemyDir * (15 + enemySpeed * 5) })
        }
      }
      ...
      if (frame % 55 === 0) {
        ...
        if (shooters.length > 0) {
          ...
          enemyBullets.push({ x: s.x, y: s.y + 10, speed: 2.5 })
        }
      }
      ...
      for (let i = bullets.length - 1; i >= 0; i--) {
        ...
        for (const e of enemies) {
          if (!e.alive) continue
          if (
            Math.abs(bullets[i].x - e.x) < 12 &&
            Math.abs(bullets[i].y - e.y) < 12
          ) {
            e.alive = false
            hit = true
            explosions.push({
